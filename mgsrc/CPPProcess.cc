...
#include "HelAmps_HiggsCharac_UFO.h"

using namespace MG5_HiggsCharac_UFO; 
...
void CPPProcess::initProc(string param_card_name) 
{
  ...
  pars = Parameters_HiggsCharac_UFO::getInstance(); 
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_gg_xxwpwm_epvemumvmx(); 
...
double CPPProcess::matrix_gg_xxwpwm_epvemumvmx() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 2; 
